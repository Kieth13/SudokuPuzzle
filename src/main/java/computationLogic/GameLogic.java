...
import static domain.SudokuGame.GRID_BOUNDARY;
...
public class GameLogic {
    ...
    private static boolean squaresAreInvalid(int[][] grid) {
        if (rowOfSquaresInvalid(Rows.TOP, grid)) return true;
        if (rowOfSquaresInvalid(Rows.MIDDLE, grid)) return true;
        if (rowOfSquaresInvalid(Rows.BOTTOM, grid)) return true;
        return false;
    }

    private static boolean rowOfSquaresInvalid(Rows value, int[][] grid) {
        switch(value) {
            case TOP:
